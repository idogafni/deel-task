...
app.get("/contracts/:id", getProfile, async (req, res) => {
  ...
  const contract = await Contract.findOne({
    where: {
      id,
      [Op.or]: [
        { ClientId: req.profile.get("id") },
        { ContractorId: req.profile.get("id") },
      ],
    },
  });
...
app.put("/jobs/:job_id/pay", getProfile, async (req, res) => {
  ...
  if (job.price <= currentBalance) {
    ...
    if (!contractor) res.json({ err: "No contractor found" });
    ...
    res.json({
      err: "Not enough balance in order to pay for job id : " + job_id,
    });
  }
  res.status(200).end();
});
